#!/usr/bin/env python3
"""
Rebuild odia_data.js to mirror the Hindi/Maithili phrase+word sets (same English keys).
Uses deep-translator (English→Odia) + indic-transliteration Oriya → IAST for `roman`.
"""
import json
import os
import re
...
# Windows consoles may lack UTF-8 stdout
...
from indic_transliteration.sanscript import IAST, ORIYA, transliterate

ROOT = os.path.dirname(os.path.abspath(__file__))
CACHE_PATH = os.path.join(ROOT, "_odia_translate_cache.json")
HINDI_PATH = os.path.join(ROOT, "hindi_data.js")
OUT_PATH = os.path.join(ROOT, "odia_data.js")
...
def romanize_or(od: str) -> str:
    rom = transliterate(od, ORIYA, IAST)
...
def translate_pair(en: str) -> tuple[str, str]:
    ...
    try:
        ...
        print("translit error:", repr(en)[:60], repr(od)[:60], e)
        rom = ""
    cache[en] = [od, rom]
...
def repl(m: re.Match) -> str:
    ...
    od_t, rom = translate_pair(en)
    hint = m.group(4) or ""
    return (
        "{ en: "
        + json.dumps(en, ensure_ascii=False)
        + ", mr: "
        + json.dumps(od_t, ensure_ascii=False)
        + ", roman: "
...
def fix_maithili_question_safe(text: str) -> str:
    od_q = "ଆପଣ ଓଡ଼ିଆ କହିପାରିବେ କି?"
    rom_q = romanize_or(od_q)

    def _sub(_m: re.Match) -> str:
        hint = _m.group(3) or ""
        return (
            '{ en: "Do you speak Maithili?", mr: '
            + json.dumps(od_q, ensure_ascii=False)
            + ", roman: "
...
def main():
    ...
    updated = updated.replace("const HINDI_PHRASES", "const ODIA_PHRASES")
    updated = updated.replace("const HINDI_DICTIONARY", "const ODIA_DICTIONARY")
    updated = updated.replace("Hindi phrases", "Odia phrases")
    updated = updated.replace("data_hindi.json", "data_odia.json")
