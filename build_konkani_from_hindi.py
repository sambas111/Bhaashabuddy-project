#!/usr/bin/env python3
"""
Rebuild konkani_data.js to mirror the Hindi/Maithili phrase+word sets (same English keys).
Uses deep-translator (English→Konkani, code `gom`) + indic-transliteration Devanagari → IAST for `roman`.
"""
import json
import os
import re
...
CACHE_PATH = os.path.join(ROOT, "_konkani_translate_cache.json")
HINDI_PATH = os.path.join(ROOT, "hindi_data.js")
OUT_PATH = os.path.join(ROOT, "konkani_data.js")

translator = GoogleTranslator(source="en", target="gom")
...
FIX_MAITHILI_MR = "तुमी मैथिली उलयतात?"
...
def translate_pair(en: str) -> tuple[str, str]:
    ...
    try:
        ...
        print("translit error:", repr(en)[:60], repr(kok)[:60], e, flush=True)
        rom = ""
    cache[en] = [kok, rom]
...
def repl(m: re.Match) -> str:
    ...
    kok_t, rom = translate_pair(en)
    hint = m.group(4) or ""
    return (
        "{ en: "
        + json.dumps(en, ensure_ascii=False)
        + ", mr: "
        + json.dumps(kok_t, ensure_ascii=False)
        + ", roman: "
...
def main():
    ...
    updated = updated.replace("const HINDI_PHRASES", "const KONKANI_PHRASES")
    updated = updated.replace("const HINDI_DICTIONARY", "const KONKANI_DICTIONARY")
    updated = updated.replace("Hindi phrases", "Konkani phrases")
    updated = updated.replace("data_hindi.json", "data_konkani.json")
