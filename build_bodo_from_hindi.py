#!/usr/bin/env python3
"""
Rebuild bodo_data.js to mirror the Hindi/Maithili phrase+word sets (same English keys).

Google Translate (and deep-translator) do not expose Bodo ISO 639-3 `brx`; the mobile and
gtx endpoints return English for `tl=brx`. This build uses English→Hindi (`hi`) for `mr`:
Devanagari matches the Bodo tab script, and app `speechLang` for Bodo is already `hi-IN`.

Romanization: Devanagari → IAST via indic-transliteration (same as other Devanagari sets).
"""
import json
import os
import re
...
CACHE_PATH = os.path.join(ROOT, "_bodo_translate_cache.json")
HINDI_PATH = os.path.join(ROOT, "hindi_data.js")
OUT_PATH = os.path.join(ROOT, "bodo_data.js")
...
FIX_MAITHILI_MR = "क्या आप मैथिली बोलते हैं?"
...
def translate_pair(en: str) -> tuple[str, str]:
    ...
    for attempt in range(TRANSLATE_RETRIES):
        ...
        try:
            hi_txt = translator.translate(en)
            break
    ...
    try:
        ...
        print("translit error:", repr(en)[:60], repr(hi_txt)[:60], e, flush=True)
        rom = ""
    cache[en] = [hi_txt, rom]
...
def main():
    ...
    updated = updated.replace("const HINDI_PHRASES", "const BODO_PHRASES")
    updated = updated.replace("const HINDI_DICTIONARY", "const BODO_DICTIONARY")
    updated = updated.replace("Hindi phrases", "Bodo phrases")
    updated = updated.replace("data_hindi.json", "data_bodo.json")
