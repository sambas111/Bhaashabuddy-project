#!/usr/bin/env python3
"""
Rebuild sanskrit_data.js to mirror the Hindi/Maithili phrase+word sets (same English keys).
Uses deep-translator (English→Sanskrit) + indic-transliteration Devanagari → IAST for `roman`.
"""
import json
import os
import re
...
CACHE_PATH = os.path.join(ROOT, "_sanskrit_translate_cache.json")
HINDI_PATH = os.path.join(ROOT, "hindi_data.js")
OUT_PATH = os.path.join(ROOT, "sanskrit_data.js")
...
# Formal plural polite; stable for shared “Do you speak Maithili?” English key
FIX_MAITHILI_MR = "किं भवन्तः मैथिलीं वदन्ति?"
...
def translate_pair(en: str) -> tuple[str, str]:
    ...
    for attempt in range(TRANSLATE_RETRIES):
        ...
        try:
            sa_txt = translator.translate(en)
            break
    ...
    try:
        ...
        print("translit error:", repr(en)[:60], repr(sa_txt)[:60], e, flush=True)
        rom = ""
    cache[en] = [sa_txt, rom]
...
def repl(m: re.Match) -> str:
    ...
    sa_t, rom = translate_pair(en)
    hint = m.group(4) or ""
    return (
        "{ en: "
        + json.dumps(en, ensure_ascii=False)
        + ", mr: "
        + json.dumps(sa_t, ensure_ascii=False)
        + ", roman: "
...
def main():
    ...
    updated = updated.replace("const HINDI_PHRASES", "const SANSKRIT_PHRASES")
    updated = updated.replace("const HINDI_DICTIONARY", "const SANSKRIT_DICTIONARY")
    updated = updated.replace("Hindi phrases", "Sanskrit phrases")
    updated = updated.replace("data_hindi.json", "data_sanskrit.json")
