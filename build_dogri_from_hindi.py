#!/usr/bin/env python3
"""
Rebuild dogri_data.js to mirror the Hindi/Maithili phrase+word sets (same English keys).
Uses deep-translator (English→Dogri, code `doi`) + indic-transliteration Devanagari → IAST for `roman`.
"""
import json
import os
import re
...
CACHE_PATH = os.path.join(ROOT, "_dogri_translate_cache.json")
HINDI_PATH = os.path.join(ROOT, "hindi_data.js")
OUT_PATH = os.path.join(ROOT, "dogri_data.js")

translator = GoogleTranslator(source="en", target="doi")
...
FIX_MAITHILI_MR = "क्या तुसीं मैथिली बोलदे हो?"
...
def translate_pair(en: str) -> tuple[str, str]:
    ...
    try:
        ...
        print("translit error:", repr(en)[:60], repr(dg)[:60], e, flush=True)
        rom = ""
    cache[en] = [dg, rom]
...
def repl(m: re.Match) -> str:
    ...
    dg_t, rom = translate_pair(en)
    hint = m.group(4) or ""
    return (
        "{ en: "
        + json.dumps(en, ensure_ascii=False)
        + ", mr: "
        + json.dumps(dg_t, ensure_ascii=False)
        + ", roman: "
...
def main():
    ...
    updated = updated.replace("const HINDI_PHRASES", "const DOGRI_PHRASES")
    updated = updated.replace("const HINDI_DICTIONARY", "const DOGRI_DICTIONARY")
    updated = updated.replace("Hindi phrases", "Dogri phrases")
    updated = updated.replace("data_hindi.json", "data_dogri.json")
